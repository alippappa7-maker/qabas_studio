                        ...
                        if (newUserName.isNotBlank() && newUserEmail.isNotBlank()) {
                            ...
                            coroutineScope.launch {
                                ...
                                if (SupabaseServices.isSupabaseAvailable) {
                                    val sbSaved = SupabaseServices.Database.saveUser(
                                        id = newId,
                                        name = newUser.name,
                                        email = newUser.email,
                                        type = newUser.type
                                    )
                                    if (sbSaved) saved = true
                                }
                                if (CloudServices.isFirebaseInitialized) {
                                    val fbSaved = runCatching {
                                ...
                                Toast.makeText(
                                    context,
                                    if (saved) "تمت الإضافة وحفظها سحابياً ✅" else "أُضيف محلياً فقط",
                                    Toast.LENGTH_LONG
                                ).show()
                            }
                        }
            ...
            confirmButton = {
                TextButton(onClick = {
                    ...
                    coroutineScope.launch {
                        var success = false
                        if (SupabaseServices.isSupabaseAvailable) {
                            val sbOk = SupabaseServices.Database.updateUserRole(user.id, newType)
            ...
            confirmButton = {
                TextButton(onClick = {
                    ...
                    coroutineScope.launch {
                        var success = false
                        if (SupabaseServices.isSupabaseAvailable) {
                            val sbOk = SupabaseServices.Database.updateUserSuspension(user.id, newStatus)
                            if (sbOk) success = true
                        }
                        if (CloudServices.isFirebaseInitialized) {
                            val fbOk = CloudServices.Database.updateUserSuspension(user.id, user.email, newStatus)
                            if (fbOk) success = true
                        }
                        if (!success && !SupabaseServices.isSupabaseAvailable && !CloudServices.isFirebaseInitialized) {
                            success = true
